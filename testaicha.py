...
# Function to prompt the user to either set the hour, reset the alarm or quit
def ask_to_reset_or_quit():
    while True:
        choice = input("Do you want to reset the time? Type 'S' to reset or 'Q' to quit: ").strip().upper()
        if choice == 'S':
            return True
        elif choice == 'Q':
            ...
            print("Invalid input. Please type 'S' or 'Q'.")


# Function to ask if the user wants to reset the alarm
def ask_to_reset_alarm():
    while True:
        choice = input("Do you want to reset the alarm? Type 'S' to reset or 'Q' to quit: ").strip().upper()
        if choice == 'S':
            return True
        elif choice == 'Q':
            print("Quitting the program...")
...
try:
    ...
    if ask_to_reset_or_quit():
        ...
        sys.exit()  # Exit if the user chooses to quit

# Step 2: Display reset time dynamically
try:
    display_reset_time(hour, minute, second)
except KeyboardInterrupt:
    print("\nYou interrupted the reset time display.")
    # Ask if they want to reset the alarm after resetting the time
    if ask_to_reset_alarm():
